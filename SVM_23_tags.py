...
##Set default filename for import.
defaultFile="FibratesTrain.csv"
...
##Define whether to use decisions (0 or 1) or raw scores when reconciling tags
##into assignments.
scoresOrDecision=0
...
class SVMdataStructure:
    ...
    Target_names=list()
    def __init__(self,ID_all,content_all,assignment_all,ID_train,tags_all):
        ...
        ##Define the Target attribute (the assignments or the tags.)
...
def createData():
    ...
    try:
        with open(trainFile,encoding="ISO-8859-1") as csvfile:
            ...
            for row in readCSV:
                ...
                goldTags.append(row[inputColGoldTags])
    except:
    ...
    return(trainingIDlist,contents,goldAssignmentBool,goldTags)

## Define which tags are IN or EX, and check that this is consistent.
def DefineTagAssignment(tagsList,assignmentList):
    ##Extract unique tags.
    print("Extracting Unique tags...")
    uniqueTags=list(set(tagsList))
    uniqueTags.sort()
    ...
    for i in range(0,len(tagsList)):
        assDict[tagsList[i]].append(assignmentList[i])
    for tag in assDict:
...
def CreateClassifierResultsDictionary(tagAssignments,tagsList):
    print("Creating results Dictionary...")
    outputDict=dict()
    for tag in tagAssignments:
        outputDict[tag]={"score":[],"decision":[],"gold":[],"kappa":[],
        "recall":[],"threshold":[]}


    #print("outputDict: ",outputDict)
    ##create a boolean list for each tag as to whether the gold assignment is
    ##that tag or not.
    ##first make all entries for all tags 0.
    for tag in outputDict:
        outputDict[tag]["gold"]=[0]*len(tagsList)
    ##then change them to 1 if that tag matches the gold tag at that position.
    for i in range(0,len(tagsList)):
        for tag in outputDict:
            if tagsList[i]==tag:
                outputDict[tag]["gold"][i]=1

    return(outputDict)

def CreateReconciledResultsDictionary():
    outputDict=dict()
    outputDict["INEXgold"]=[]
    outputDict["INscore"]=[]
    outputDict["EXscore"]=[]
    outputDict["INEXscore"]=[]
    outputDict["INEXdecision"]=[]
    outputDict["INEXdecisionKappa"]=0
    outputDict["INEXdecisionRecall"]=0
    outputDict["INEXdecisionthreshold"]=0
    return(outputDict)
...
def CreateTrainingIDsLists(trainingIDlist,randSeed):
    print("Creating training ID lists...")
    random.seed(randSeed)
...
##review.
def SVM_recommendReview(SVMdata,reconciledResults):
    print("Recommending articles for review...")
    k=0
    for i in range(0,len(SVMdata.ID_all)):
        #print(i)
        if SVMdata.assignment_all[i]==0 and reconciledResults["INEXdecision"][i]==1:
...
def exportRecommendations(SVMdata,reconciledResults):
    ...
    for i in range(0,len(SVMdata.ID_all)):
        ...
        gAssignment.append(reconciledResults["INEXgold"][i])
        abs.append(SVMdata.content_all[i])
        #SVMsumVotes.append(reconciledResults["INEXdecision"][i])
        #SVMnumVotes.append(SVMdata.SVMvote_count[i])
        SVMmeanVote.append(reconciledResults["INEXscore"][i])
        sAssignment.append(reconciledResults["INEXdecision"][i])
    ...
    #print("sum_gAssignment: ",sum(gAssignment[1:]))
    #input("press ENTER")
...
##Take a dictionary object containing predicted assignments and kappa values for
##each tag and a dictionary of which tags are includes or excludes and export a
##list of include scores for each abstract.
def reconcileTagsToAssignments(classifierResults,tagAssignments,reconciledResults,inputDataLists):

    ##first erase any existing assignment values.
    for tag in classifierResults:
        vectLen=len(classifierResults[tag]["gold"])
    reconciledResults["INscore"]=[0]*vectLen
    reconciledResults["EXscore"]=[0]*vectLen
    reconciledResults["INEXscore"]=[0]*vectLen
    reconciledResults["INEXdecision"]=[0]*vectLen
    reconciledResults["INEXgold"]=inputDataLists[2]
    ##Now calculate the IN and EX scores based on the tag decisions, the kappa
    ##for that tag, and the corresponding assignment of that tag.
    if scoresOrDecision==1:
        for tag in classifierResults:
            #print(tag)
            ##for each position in "decision",
            for i in range(0,len(classifierResults[tag]["decision"])):
                if classifierResults[tag]["decision"][i]==1:
                    if tagAssignments[tag]==1:
                        #print("vote for IN")
                        reconciledResults["INscore"][i]+=classifierResults[tag]["kappa"]
                    elif tagAssignments[tag]==0:
                        reconciledResults["EXscore"][i]+=classifierResults[tag]["kappa"]
                else:
                    if tagAssignments[tag]==1:
                        #print("vote for EX")
                        reconciledResults["EXscore"][i]+=classifierResults[tag]["kappa"]
        ...
        for tag in classifierResults:
            ...
            for i in range(0,len(classifierResults[tag]["decision"])):
                if tagAssignments[tag]==1:
                    #print("vote for IN")
                    reconciledResults["INscore"][i]+=classifierResults[tag]["kappa"]*classifierResults[tag]["score"][i]
                else:
                    #print("vote for EX")
                    reconciledResults["EXscore"][i]+=classifierResults[tag]["kappa"]*classifierResults[tag]["score"][i]



    #print('reconciledResults["INscore"]',reconciledResults["INscore"])
    #print('reconciledResults["EXscore"]',reconciledResults["EXscore"])
    ##Now take a ratio of these summed IN and EX scores to get a final INEXscore.
    ##If using a decision system (i.e. 0 or 1 for each tag) then this will cause some
    ##div0 errors if some articles get no decisions for EX (the denominator),
    ##or for IN (the numerator) and so give nan or Inf values. these need to be
    ##replaced. nan for the lowest score, inf for the highest.
    ##Set staring values for max and min score.
    minScore=1000
    maxScore=0
    for i in range(0,len(reconciledResults["INscore"])):
        if reconciledResults["INscore"][i]==0:
            reconciledResults["INEXscore"][i]="min"
        elif reconciledResults["EXscore"][i]==0:
            reconciledResults["INEXscore"][i]="max"
        else:
            reconciledResults["INEXscore"][i]=reconciledResults["INscore"][i]/reconciledResults["EXscore"][i]
            if reconciledResults["INEXscore"][i]>maxScore:
                maxScore=reconciledResults["INEXscore"][i]
            if reconciledResults["INEXscore"][i]<minScore:
                minScore=reconciledResults["INEXscore"][i]
    #print(reconciledResults["INEXscore"])
    #print("minScore: ",minScore)
    #print("maxScore: ",maxScore)


    ##Now replace inf and nan scores with maxScore and minScore
    #print('Current reconciledResults["INEXscore"]:',reconciledResults["INEXscore"])
    for i in range(0,len(reconciledResults["INEXscore"])):
        if reconciledResults["INEXscore"][i]=='min':
            reconciledResults["INEXscore"][i]=minScore
        if reconciledResults["INEXscore"][i]=='max':
            reconciledResults["INEXscore"][i]=maxScore
    #print('New reconciledResults["INEXscore"]:',reconciledResults["INEXscore"])
    #input("Press ENTER")
##similarly to the individual models, applies a slding threshold to the overall
##INEXscores to proeuce overall INEXdecisions.
def ApplyThresholdingToAssignmentScores(reconciledResults):
    ##Now apply thresholding and kappa calculations to assign overall INEXdecisions.
    print("adjusting final INEXdecisions for kappa...")
    currentThreshold=max(reconciledResults["INEXscore"])
    print("starting threshold: ",currentThreshold)
    ##Define a loop that will measure the kappa, and then incrementally decrease
    ##the threshold until all articles get through.
    ##First define three variables to store the current best threshold,
    ## the best Kappa value from it, and the best Recall value from it.
    ...
    bestThreshold=0
    currentINprop=0
    while currentINprop<1 and currentThreshold>0.0001:
        #print("Searching...")
        ##First use the score and the current threshold to make an IN-EX
        ## assignemnt for each article.
        currentIN=0
        currentEX=0
        for i in range(0,len(reconciledResults["INEXscore"])):
            if reconciledResults["INEXscore"][i]>=currentThreshold:
                reconciledResults["INEXdecision"][i]=1
                currentIN+=1
            else:
                reconciledResults["INEXdecision"][i]=0
                currentEX+=1
        ##Calculate the current proprotion included.
        currentINprop=currentIN/(currentIN+currentEX)
        ##Calculate the Recall and kappa
        inputGoldAssListBool=reconciledResults["INEXgold"]
        SVMassListBool=list()
        for i in range(0,len(reconciledResults["INEXscore"])):
            SVMassListBool.append(reconciledResults["INEXdecision"][i])
        #print("inputGoldAssListBool: ",inputGoldAssListBool)
        #print("SVMassListBool: ",SVMassListBool)
        currentRecall=metrics.recall_score(inputGoldAssListBool,SVMassListBool)
        currentKappa=metrics.cohen_kappa_score(inputGoldAssListBool,SVMassListBool)
    ...
    ## final classifier, but shouldn't add much to runtime to do each time.
    for i in range(0,len(reconciledResults["INEXscore"])):
        if reconciledResults["INEXscore"][i]>=bestThreshold:
            reconciledResults["INEXdecision"][i]=1
        else:
            reconciledResults["INEXdecision"][i]=0
...
tagAssignments=DefineTagAssignment(inputDataLists[3],inputDataLists[2])
print("tagAssignments: ",tagAssignments)
classifierResults=CreateClassifierResultsDictionary(tagAssignments,inputDataLists[3])
#print("classifierResults dictionary: ",classifierResults)
...
####################################################################
##Run a loop to optimise and run the classifier for each tag.
for tag in classifierResults:
    print("Running classifiers for ",tag,"...")
    ##Pick the articles to be used for training and testing.
    trainingIDlist=PickTrainingArticles(inputDataLists,randSeed,classifierResults[tag]['gold'])
    ...
    ##Create the SVM data object. By default the Target attribute is a list of gold
    ##assigments.
    print("Creating SVMdataStructure...")
    SVMdata=SVMdataStructure(inputDataLists[0],wordsSetList,classifierResults[tag]['gold'],trainingIDlist,inputDataLists[3])
    #print(SVMdata.ID_all)
    #print(SVMdata.content_all)
    #print(SVMdata.assignment_all)
    #print(SVMdata.tags_all)
    ...
    performance=AdjustThresholdForKappa(SVMdata,startingThreshold)
    ##Place results back in classifierResults dictionary.
    classifierResults[tag]['score']=SVMdata.SVMvote_mean
    classifierResults[tag]['decision']=SVMdata.SVMvote_decision
    classifierResults[tag]['kappa']=performance["Kappa"]
    classifierResults[tag]['recall']=performance["Recall"]
    classifierResults[tag]['threshold']=performance["Threshold"]
    #del SVMdata
    #cg.collect()

##Display Results
print("\nResults of multi-tag classifiers:")
for tag in classifierResults:
    print(tag)
    print("kappa: ",classifierResults[tag]['kappa'])
    print("recall: ",classifierResults[tag]['recall'])
    print("threshold: ",classifierResults[tag]['threshold'])

##Now combine the results for all tags into a single score for being an include.
##make the SVMdata INEX gold assignments equal to the true gold INEX assignments
##(rather than the gold tag INEX values from the last tag.)
#print("inputDataLists[2]",inputDataLists[2])
#print("sum",sum(inputDataLists[2]))
#input("press Enter")
SVMdata.assignment_all=inputDataLists[2]
#print("sum",sum(SVMdata.assignment_all))

reconciledResults=CreateReconciledResultsDictionary()
reconcileTagsToAssignments(classifierResults,tagAssignments,reconciledResults,inputDataLists)
ApplyThresholdingToAssignmentScores(reconciledResults)


##Make review recommendations based on analysis and threshold.
numReturn=SVM_recommendReview(SVMdata,reconciledResults)
...
exportRecommendations(SVMdata,reconciledResults)
