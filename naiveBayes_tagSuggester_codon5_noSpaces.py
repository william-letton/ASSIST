...
for abstract in all_abstracts_lower:
    split=list()
    for i in range(0,len(abstract)-4):
        codon=abstract[i]+abstract[i+1]+abstract[i+2]+abstract[i+3]+abstract[i+4]
        split.append(codon)
    for word in split:
...
for i in screening_codes_key:
    ...
    for l in range(0,count_all_abstracts):
        ...
        if bool[l]==True:
            split=list()
            for pos in range(0,len(all_abstracts_lower[l])-4):
                codon=all_abstracts_lower[l][pos]+all_abstracts_lower[l][pos+1]+all_abstracts_lower[l][pos+2]+all_abstracts_lower[l][pos+3]+all_abstracts_lower[l][pos+4]
                split.append(codon)
            for word in split:
