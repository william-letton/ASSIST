...
##at the expense of accuracy and Kappa.
minRecall=0.8
...
def AdjustForRecall(articles,startingThreshold,minRecall):
...
def TripleParamsOptimiser(params,SVMdata_full,trainingIDgroups):
    ...
    for param in params:
        ...
        SVMmetrics=AdjustForRecall(inputArticles,startingThreshold,minRecall)
...
##of dictionaries, each with a pairwise combination of 2 C and gamma parameter values.
## the range argument will give the number of orders of magnitude the values will span.
def CreateInitialParams(logRange=10):
    ##Define a list of real numbers to use as exponents
    expValues=[0-(logRange/2),(logRange/2)]
...
##optimisation.
def UpdateParamsGrid(optParams,logRange):
    print("Updating testing parameters...")
    print("logRange is: ",logRange)
    ...
    CexpValues=[Copt-(logRange/2),Copt+(logRange/2)]
    gammaExpValues=[gammaOpt-(logRange/2),gammaOpt+(logRange/2)]
    ##Use these as exponents for a log scale.
...
def NestedParamsOptimisation(paramsGrid,SVMdata_full,trainingIDgroups,initialParamRange,nestedIterations=3,refineRangeFactor=2):
    ...
    while count<nestedIterations:
        ...
        paramsGrid=UpdateParamsGrid(optParams,logRange=(initialParamRange/(refineRangeFactor**count)))
...
paramsGrid=CreateInitialParams(logRange=initialParamRange)
...
AdjustForRecall(inputArticles,startingThreshold,minRecall)
