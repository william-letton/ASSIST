...
class Article:
    ...
    def suggestReview(self):
        self.recommendReview=True
...
def CalculateProbIN(articles,probDict_codon4,probDict_word,probDict_word2,kappaDict,tagAssignmentDict):
    ...
    for ID in probDict_codon4:
        ...
        ## means something
        kappaTotal=kappaDict["codon4"]["allTags"]+kappaDict["word"]["allTags"]+kappaDict["word2"]["allTags"]
        inDict[ID]=((inProb_codon4+inProb_word+inProb_word2)/(kappaTotal))
    #print("INdict ",inDict)
...
##If the gold assignment is EX and the recommended assignment is IN for any of the methods, set the
##recommendReview attribute to True. Returns the number of articles for review.
def MakeReviewRecommendations(articles):
    print("Making article review recommendations based on method tags...")
    k=0
    for article in articles:
        if (article.assignment_suggested=="IN" or article.assignment_nb_codon4=="IN" or article.assignment_nb_word=="IN" or article.assignment_nb_word2=="IN") and article.goldAssignment!="IN":
            article.suggestReview()
            k=k+1
    return(k)
def MakeReviewRecommendations_additional(articles,numForReview):
    ...
    print("     Articles recommended for review based on tags: ",currentTotal)
    print("     Articles requested for review: ",numForReview)
    ##check that the requested number of articles isn't higher than the current
    ##number of gold excludes.
    goldExcludes=0
    for article in articles:
        if article.goldAssignment!="IN":
            goldExcludes=goldExcludes+1
    if numForReview>goldExcludes:
        print("ERROR: more articles requested for review than were marked as Excludes by human screening. Only a maximum of",goldExcludes,"can be returned.")
        input("\nPress ENTER to exit.")
        quit()
    ##Create a list from 1 to the number of articles.
    ##Create a loop that will add the top ranked article, check the current number of
    ##articles for review, if it is less than the desired number it loops again.
    loopNum=1
    while currentTotal<numForReview:
        #print("Loop number: ",loopNum)
        for article in articles:
            if article.prob_IN_rank==loopNum:
                if article.recommendReview==False and article.goldAssignment!="IN":
    ...
    for article in articles:
        if article.recommendReview==True:
            currentTotal=currentTotal+1
    print("Total articles after additional processing: ",currentTotal)
...
##Ask for how many articles the user wants returned from the analysis for review.
numForReview=int(input("How many articles would you like returned for review?"))
...
CalculateProbIN(testingArticles,NBtagProbs_codon4,NBtagProbs_word,NBtagProbs_word2,kappa_methods_dict,tagAssignmentDict)
#for article in testingArticles:
...
##Recommend articles for review based on goldAssignments and suggestedAssignments.
countReview=MakeReviewRecommendations(testingArticles)
#for article in testingArticles:
...
print("Analysis found",countReview,"articles for review based on recommended tags.")

##Make further review recommendations based on INprob values
MakeReviewRecommendations_additional(testingArticles,numForReview)
