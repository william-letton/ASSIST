...
def AdjustForRecall(articles,startingThreshold,minRecall):
    ...
    ##threshold until the minimum recall is reached.
    while recallMet==False:
        for article in articles:
    ...
    ##Now print the final recall and Kappa
    inputGoldAssListBool=list()
    SVMassListBool=list()
    for article in inputArticles:
        if article.goldAssignment=="IN":
    ...
    currentRecall=metrics.recall_score(inputGoldAssListBool,SVMassListBool)
    currentKappa=metrics.cohen_kappa_score(inputGoldAssListBool,SVMassListBool)
    print("Final recall: ",currentRecall)
    print("Final Kappa: ", currentKappa)
    return({"Recall":currentRecall,"Kappa":currentKappa,"Threshold":currentThreshold})
...
##define initial log parameter range.
initialParamRange=8
paramsGrid=CreateInitialParams(range=initialParamRange)
print("paramsGrid: ",paramsGrid)

SVMparams=NestedParamsOptimisation(paramsGrid,SVMdata_full,trainingIDgroups,initialParamRange,nestedIterations=3,refineRangeFactor=2)
