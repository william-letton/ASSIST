...
## from a method, and a dictionary of which tags are IN or EX.
def CalculateProbIN(articles,probDict,tagAssignmentDict):
    ...
    for ID in probDict:
        ...
        inTemp=0
        outTemp=0
        for tag in probDict[ID]:
            if tagAssignmentDict[tag]=="IN":
                inTemp=inTemp+probDict[ID][tag]
            else:
                outTemp=outTemp+probDict[ID][tag]
        inProb=inTemp/(inTemp+outTemp)
        inDict[ID]=inProb
    #print("INdict ",inDict)
    ...
    for article in articles:
        ...
        #print("article prob_IN: ",article.prob_IN)
...
def MakeReviewRecommendations(articles):
    k=0
    for article in articles:
        if (article.assignment_suggested=="IN" or article.assignment_nb_codon4=="IN" or article.assignment_nb_word=="IN") and article.goldAssignment!="IN":
...
##given as an output of the codon4 method.
NB_codon4_INProbs=CalculateProbIN(testingArticles,NBtagProbs_codon4,tagAssignmentDict)
...
print("Analysis found",countReview,"articles for review.")
