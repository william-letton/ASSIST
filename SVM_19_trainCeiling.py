...
class SVMdata:
    ...
    Target_names=list()
    #Votes=list()
    def __init__(self,DESCR,IDs,Data,Feature_names,Filename,Target,Target_names,Training_articles,Training_articles_IDs):
        self.DESCR=DESCR
        self.IDs=IDs
        self.Data=Data
        ...
        self.Training_articles_IDs=Training_articles_IDs
...
def createSVMdata(articles,dataUnits):
    ...
    Training_articles_IDs=list()

    ##Process the articles to produce a list of training articles (1 for IN, 0 for EX),
    ##Along with a corresponding list of their IDs.
    for article in articles:

        if article.SVMtrainIN==True:
            Training_articles.append(1)
            Training_articles_IDs.append(article.ID)
        elif article.SVMtrainEX==True:
    ...
    obOut=SVMdata(DESCR,IDs,Data,Feature_names,Filename,Target,Target_names,Training_articles,Training_articles_IDs)
    return(obOut)
...
def CreateTrainingData(dataset,numClassifiers=2):
    ...
    print("Total abstracts used for training: ",(numClassifiers*absPerClassifier))
...
def RunClassifiers(SVMdata,trainingIDgroups,params,articles):
    ...
    def RunClassifier (SVMdata,trainingIDgroup,params,articles):
        ...
        clf = svm.LinearSVC(C=params['C'],random_state=7)
        ...
        ##First split Xtest into chuncks of 1000 to avoid running out of memory.
        chunkLength=100
        #print("chunkLength: ",chunkLength)
        #print("len(testX): ",len(Xtest))
        Xtest_chunks=[Xtest[x:x+chunkLength]for x in range(0,len(Xtest),chunkLength)]
        ##Also Split the YID list to match the Xtest chuncks
        YID_chunks=[YID[x:x+chunkLength]for x in range(0,len(YID),chunkLength)]
        #print("len(testX_chunks): ",len(Xtest_chunks))
        #for i in Xtest_chunks:
        #    print(len(i))
        #for i in YID_chunks:
        #    print(len(i))

        print("Making predictions...")
        for i in range(0,len(Xtest_chunks)):
            Yout = clf.decision_function(Xtest_chunks[i])
            #print("Yout: ",Yout)
            ...
            ## articles object. Use the fact that the index of the current Xtest chunk
            ## corresponds to the same index in the corresponding YID chunk.
            for article in inputArticles:
                for pos in range(0,len(Ypred)):
                    #print("Current article ID: ",article.ID)
                    if article.ID == YID_chunks[i][pos]:
