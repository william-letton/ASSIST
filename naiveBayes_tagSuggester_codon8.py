...
for abstract in all_abstracts_lower:
    split=list()
    for i in range(0,len(abstract)-7):
        codon=abstract[i]+abstract[i+1]+abstract[i+2]+abstract[i+3]+abstract[i+4]+abstract[i+5]+abstract[i+6]+abstract[i+7]
        split.append(codon)
    for word in split:
...
for i in screening_codes_key:
    ...
    for l in range(0,count_all_abstracts):
        ...
        if bool[l]==True:
            split=list()
            for pos in range(0,len(all_abstracts_lower[l])-7):
                codon=all_abstracts_lower[l][pos]+all_abstracts_lower[l][pos+1]+all_abstracts_lower[l][pos+2]+all_abstracts_lower[l][pos+3]+all_abstracts_lower[l][pos+4]+all_abstracts_lower[l][pos+5]+all_abstracts_lower[l][pos+6]+all_abstracts_lower[l][pos+7]
                split.append(codon)
            for word in split:
...
fields =['abstract','ID','suggestedTag','goldTag']
