...
def OptimiseParams(dataset,optimisingProp=0.5):
    ...
    def FindParams(dataset,optimisingProp):
        ...
        cv = StratifiedShuffleSplit(n_splits=5, train_size=optimisingProp, random_state=42)
    ...
    gridCrangeGammaRange=FindParams(dataset,optimisingProp)
...
def CreateTrainingData(dataset,numClassifiers=5,trainProp=1):
...
FibratesData=createSVMdata(trainingArticles,"word")

##Train Train classifiers.
bestParams=OptimiseParams(FibratesData,optimisingProp=0.2)
print(bestParams)

##Create the lists of training data.
trainingIDgroups=CreateTrainingData(FibratesData,numClassifiers=5,trainProp=1)
