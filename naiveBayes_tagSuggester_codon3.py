...
for abstract in all_abstracts_lower:
    split=list()
    for i in range(0,len(abstract)-2):
        codon=abstract[i]+abstract[i+1]+abstract[i+2]
        split.append(codon)
    for word in split:
...
for i in screening_codes_key:
    ...
    for l in range(0,count_all_abstracts):
        ...
        if bool[l]==True:
            split=list()
            for pos in range(0,len(all_abstracts_lower[l])-2):
                codon=all_abstracts_lower[l][pos]+all_abstracts_lower[l][pos+1]+all_abstracts_lower[l][pos+2]
                split.append(codon)
            for word in split:
