...
trainTestFile=input('Please enter the csv filename (including ".csv"), then press ENTER: ')
...
if trainTestFile is "":
    trainTestFile = "FibratesTrainTiny.csv"
if inputColID is "":
    inputColID = "A"
...
#print (inputColID,inputColAbstract,inputColGoldTags)
trainFile=trainTestFile
testFile=trainTestFile
...
with open(trainFile,encoding="ISO-8859-1") as csvfile:
    ...
    for row in readCSV:
        trainingID=row[inputColID]
        abstract=row[inputColAbstract]
        screening_code=row[inputColGoldTags]
...
for abstract in all_abstracts_lower:
    split=list()
    for i in range(0,len(abstract)-3):
        codon=abstract[i]+abstract[i+1]+abstract[i+2]+abstract[i+3]
        split.append(codon)
    for word in split:
...
for i in screening_codes_key:
    ...
    for l in range(0,count_all_abstracts):
        ...
        if bool[l]==True:
            split=list()
            for pos in range(0,len(all_abstracts_lower[l])-3):
                codon=all_abstracts_lower[l][pos]+all_abstracts_lower[l][pos+1]+all_abstracts_lower[l][pos+2]+all_abstracts_lower[l][pos+3]
                split.append(codon)
            for word in split:
...
##Create a dictionary of abstracts and IDs so that the IDs can be exported as
## part of the results
#print("novelIDlist: ",novelIDlist)
absIDdict=dict()
for i in range(0,len(novel_abstracts_lower)):
    absIDdict[novel_abstracts_lower[i]]=novelIDlist[i]
#print("absIDdict: ",absIDdict)

##Add the IDs to the suggestedTagsDict dictionary.
for abstract in suggestedTagsDict:
    suggestedTagsDict[abstract]["ID"]=absIDdict[abstract]
#print("suggestedTagsDict: ",suggestedTagsDict)
...
fields =['ID','goldTag','suggestedTag']
